...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFXAmount = strAnonymizeFXAmount.remove(strAnonymizeFXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFXAmount*COIN) nMaxToAnonymize = nAnonymizeFXAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFXAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFXAmount));
        strAnonymizeFXAmount = strAnonymizeFXAmount.remove(strAnonymizeFXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
