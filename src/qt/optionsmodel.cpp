...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFXAmount"))
        settings.setValue("nAnonymizeFXAmount", 1000);
    nAnonymizeFXAmount = settings.value("nAnonymizeFXAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFXAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeFXAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFXAmount:
            return QVariant(nAnonymizeFXAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeFXAmount:
            nAnonymizeFXAmount = value.toInt();
            settings.setValue("nAnonymizeFXAmount", nAnonymizeFXAmount);
            emit AnonymizeFXAmountChanged(nAnonymizeFXAmount);
            break;
        default:
            break;
        }
