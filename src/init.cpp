...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizefxcoinamount=<n> " + _("Keep N FX anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FX is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FX is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FX version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "FX server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FX") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart FX to complete") << "\n";
    ...
    nAnonymizeFXAmount = GetArg("-anonymizefxcoinamount", 0);
    if(nAnonymizeFXAmount > 999999) nAnonymizeFXAmount = 999999;
    if(nAnonymizeFXAmount < 2) nAnonymizeFXAmount = 2;
    ...
    LogPrintf("Anonymize FX Amount %d\n", nAnonymizeFXAmount);
