...
namespace {
    class CFXcoinAddressVisitor : public boost::static_visitor<bool> {
    private:
        CFXcoinAddress *addr;
    public:
        CFXcoinAddressVisitor(CFXcoinAddress *addrIn) : addr(addrIn) { }
...
bool CFXcoinAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CFXcoinAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CFXcoinAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CFXcoinAddressVisitor(this), dest);
}

bool CFXcoinAddress::IsValid() const {
...
CTxDestination CFXcoinAddress::Get() const {
    if (!IsValid())
...
bool CFXcoinAddress::GetKeyID(CKeyID &keyID) const {
...
bool CFXcoinAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CFXcoinSecret::SetKey(const CKey& vchSecret) {
...
CKey CFXcoinSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CFXcoinSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CFXcoinSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CFXcoinSecret::SetString(const std::string& strSecret) {
